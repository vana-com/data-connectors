...
function renderReadmeConnectorOverview(catalog) {
  const sources = new Map();
  for (const scope of catalog.scopes) {
    const source = sources.get(scope.sourceId) ?? {
      scopeIds: [],
      connectors: new Map(),
    };
    source.scopeIds.push(scope.scopeId);
    for (const connector of scope.fulfillment.desktop.connectors) {
      source.connectors.set(connector.id, connector.status);
    }
    sources.set(scope.sourceId, source);
  }

  const rows = [...sources.entries()].map(([sourceId, source]) => {
    const scopeIds = source.scopeIds.map((scopeId) => `\`${scopeId}\``).join(", ");
    const connectors = [...source.connectors.entries()]
      .map(([id, status]) => `${id} (${status})`)
      .join("; ");
    return `| \`${escapeCell(sourceId)}\` | ${escapeCell(scopeIds)} | ${escapeCell(connectors)} |`;
  });

  return `${readmeOverviewStart}
...
function renderReadme(readme, catalog) {
  const startIndex = readme.indexOf(readmeOverviewStart);
  const endIndex = readme.indexOf(readmeOverviewEnd);
  const hasOneStart = startIndex !== -1 && startIndex === readme.lastIndexOf(readmeOverviewStart);
  const hasOneEnd = endIndex !== -1 && endIndex === readme.lastIndexOf(readmeOverviewEnd);
  if (!hasOneStart || !hasOneEnd || endIndex < startIndex) {
    throw new Error("README.md must contain exactly one ordered generated connector overview block");
  }

  return `${readme.slice(0, startIndex)}${renderReadmeConnectorOverview(catalog)}${readme.slice(endIndex + readmeOverviewEnd.length)}`;
}
...
export function generateScopeCatalog({
  ...
  checkOrWrite(
    readmePath,
    renderReadme(readFileSync(readmePath, "utf8"), catalog),
    check,
  );
  return catalog;
}
  ...
  try {
    ...
    console.log(
      check
        ...
        : `Generated scope-catalog.json, SCOPES.md, and README.md (${catalog.scopes.length} scopes).`,
    );
  } catch (error) {
    console.error(
