...
The session persists in `~/.pdp-connect/desktop/browser-profiles/<platform>-playwright/` and is
...
2. Check if the browser profile exists:
   ```bash
   ls ~/.pdp-connect/desktop/browser-profiles/<platform>-playwright/ 2>/dev/null && echo "Profile exists" || echo "No profile"
   ```
...
ls ~/.pdp-connect/desktop/browser-profiles/<platform>-playwright/ 2>/dev/null && echo "Session profile exists" || echo "No session profile"
