...
function contribute(connectorPath, connectorDir, metadataPath) {
  ...
  if (!useGh) {
    try {
      execSync('git ls-remote https://github.com/PDP-Connect/data-connectors.git HEAD', { stdio: 'pipe' });
    } catch {
  ...
  try {
    // Fork if needed
    if (useGh) {
      try { execSync('gh repo fork PDP-Connect/data-connectors --clone=false', { stdio: 'pipe', cwd: repoRoot }); } catch {}
    }
    ...
    if (useGh) {
      ...
      const desc = metadata?.description || `Exports data from ${platform} using browser automation.`;
      const scopeList = (metadata?.scopes || [])
        .map(s => `- \`${s.scope}\`: ${s.description || s.label || s.scope}`)
        .join('\n');
      // Read report template if available
      const templatePath = path.join(repoRoot, 'reports', 'TEMPLATE.md');
      let reportSection = '';
      ...
      const prBody = [
        ...
        `## Scopes`,
        ``,
        scopeList || '_No scopes defined._',
        reportSection,
        ``,
        `---`,
        `Generated by the [pdp-connect skill](https://github.com/PDP-Connect/data-connectors/tree/main/skills/pdp-connect).`,
      ].join('\n');

      const prBodyFile = path.join(repoRoot, '.pr-body.tmp');
      fs.writeFileSync(prBodyFile, prBody);
      try {
        const prUrl = execSync(
          `gh pr create --title "${commitMsg}" --body-file ${prBodyFile} --head ${branchName}`,
          { encoding: 'utf-8', cwd: repoRoot }
        ).trim();
        console.log(`\nPR created: ${prUrl}`);
      } finally {
        try { fs.unlinkSync(prBodyFile); } catch {}
      }
    } else {
      execSync(`git push -u origin ${branchName}`, { stdio: 'pipe', cwd: repoRoot });
      console.log(`\nBranch pushed: ${branchName}`);
      console.log(`Open a PR at: https://github.com/PDP-Connect/data-connectors/compare/${branchName}`);
    }
  } catch (e) {
