...
What to do with collected data. Each recipe starts from `~/.pdp-connect/desktop/last-result.json`.
...
- **Generic:** `~/.pdp-connect/desktop/user-profile.md`
...
```bash
cp ~/.pdp-connect/desktop/last-result.json ~/backups/github-export-2026-03-10.json
```
...
cp ~/.pdp-connect/desktop/last-result.json ~/backups/<platform>-$(date +%Y-%m-%d).json
```
...
2. Before each run, copy the previous result: `cp ~/.pdp-connect/desktop/last-result.json ~/.pdp-connect/desktop/<platform>-result.json`
